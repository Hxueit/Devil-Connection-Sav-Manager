...
class DragHandler:
    ...
    def __init__(
        ...
        # 拖拽指示器列表：(item_id, original_text, after_id)
    ...
    def handle_button1_click(self, event) -> Optional[str]:
    ...
    def handle_drag_motion(self, event) -> None:
        """处理拖拽移动事件
        ...
        # 检查是否真的在拖拽（超过阈值）
        if abs(event.y - (self.drag_start_y or 0)) <= self.DRAG_THRESHOLD:
            return
    ...
    def handle_drag_end(self, event) -> None:
        """处理拖拽结束事件
    ...
    def _show_drag_indicator_line(self, target_item: str, is_dragging_down: bool) -> None:
        ...
        """
        if not self.drag_indicator_line or not self.tree.exists(target_item):
            return
        ...
        if (self.current_indicator_target == target_item and 
            ...
            self.drag_indicator_line.place(x=tree_x, y=line_y, 
                                          width=tree_width, height=3)
    ...
    def _should_process_drag(self) -> bool:
        ...
        """
        return (self.drag_start_item is not None and 
                self.is_dragging and
                self.tree.exists(self.drag_start_item))
    ...
    def _perform_drag_move(self, end_item: str) -> None:
        ...
        new_item = self.tree.insert("", insert_index, 
                                   text=item_values['text'], 
                                   values=tuple(current_values), 
                                   tags=item_values['tags'])
    ...
    def _get_data_index(self, children: List[str], tree_index: int) -> int:
        ...
        for i in range(tree_index):
            if i < len(children):
                item_id = children[i]
                item_tags = self.tree.item(item_id, "tags")
                if item_tags and ("PageHeaderLeft" not in item_tags and 
                                "PageHeaderRight" not in item_tags):
                    data_index += 1
        return data_index
    ...
    def _remove_indicator_from_item(self, item_id: str) -> None:
        ...
        self.drag_indicators = [(iid, orig, aid) for iid, orig, aid in self.drag_indicators 
                                if iid != item_id]
