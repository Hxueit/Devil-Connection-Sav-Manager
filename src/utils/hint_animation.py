...
class HintAnimation:
    ...
    def __init__(self, root: tk.Misc, target_widget: ttk.Checkbutton,
        ...
        self.wrapper = getattr(target_widget, "wrapper", None)
        self._last_trigger = 0.0

    def trigger(self) -> None:
        """播放动画；DEBOUNCE_MS 内重复触发会被忽略"""
        now = time.monotonic() * 1000
        if now - self._last_trigger < DEBOUNCE_MS or not self.wrapper:
            return
        ...
        self._shake(self._original_padx(), 0)
    ...
    def _original_padx(self) -> int:
        padx = getattr(self.target_widget, "_original_pack_info", {}).get("padx", DEFAULT_PADX)
        if isinstance(padx, (list, tuple)):
            padx = padx[0] if padx else DEFAULT_PADX
        return int(padx) if padx else DEFAULT_PADX
